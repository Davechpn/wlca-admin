...
export class worddetailsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    //Get the id url parameter which is the word id----------------------
  ...
  getDetail(id){
    //We already have word id so we use valueChanges()
    this.word = this.db.doc(this.meta.CHURCH_ID+'/cms/messages/'+id+'/').valueChanges();
    this.word.pipe(takeUntil(this.unsubscribe)).subscribe(x=>{
