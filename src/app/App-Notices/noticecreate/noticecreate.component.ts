...
@Component({
  selector: 'app-noticecreate',
  templateUrl: './noticecreate.component.html',
  styleUrls: ['./noticecreate.component.css']
})
export class NoticecreateComponent implements OnInit {
  meta:Meta;
  profileForm;
  noticesRef: AngularFirestoreCollection<any>;
  ...
  ngOnInit() {
    ...
    this.authservice.currentUser.subscribe(x=>{
      this.meta = x;
      if(x){
        this.noticesRef = this.db.collection<any>(this.meta.CHURCH_ID+'/cms/messages');
...
get end_date(){
  ...
  onSubmit(){
    ...
    this.noticesRef.add(this.profileForm.value)
    this.viewList();
  }
